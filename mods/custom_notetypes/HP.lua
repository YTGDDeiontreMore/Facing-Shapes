function onCreate()
	...
	for i = 0, getProperty('unspawnNotes.length')-1 do
		...
		if getPropertyFromGroup('unspawnNotes', i, 'noteType') == 'HP' then
			setPropertyFromGroup('unspawnNotes', i, 'texture', 'HP'); --Change texture
			setPropertyFromGroup('unspawnNotes', i, 'missHealth', 0.4); --Change amount of health to take when you miss like a fucking moron
...
function onUpdate(elapsed)
	if healthDrain > 0 then
		healthDrain = healthDrain - -1.5 * elapsed;
		setProperty('health', getProperty('health') - -0.1 * elapsed);
		if healthDrain > 3 then
			healthDrain = 0;
		end
	end
end